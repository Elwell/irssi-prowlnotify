...
#  15.12.2010 Added screen_away boolean for status flag (AE)
#
#
# usage:
#
...
Irssi::settings_add_bool('misc', 'screen_away', 0);
...
sub screen_away {
  ...
  if (Irssi::settings_get_bool($IRSSI{'name'} . '_active') == 1) {
    ...
    if ($away == 1 and $away_status != 1) {
      # set away
      Irssi::settings_set_bool('screen_away',1);
      ...
      # unset away
      Irssi::settings_set_bool('screen_away',0);
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'screen_away_crap',
        "Reset away");
      my ($server);
      foreach $server (Irssi::servers()) {
